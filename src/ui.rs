...
pub fn sw(s: &str) -> usize { UnicodeWidthStr::width(s) }

pub fn trunc(s: &str, max_cols: usize) -> String {
    let mut w = 0usize; let mut r = String::new();
    for ch in s.chars() {
        let cw = ch.width().unwrap_or(1);
        if w + cw > max_cols { if w < max_cols { r.push('~'); } break; }
        r.push(ch); w += cw;
    }
    r
}
...
pub fn padr(s: &str, cols: usize) -> String {
    let w = sw(s);
    if w >= cols { trunc(s, cols) } else { format!("{}{}", s, " ".repeat(cols - w)) }
}

pub fn padl(s: &str, cols: usize) -> String {
    let w = sw(s);
    if w >= cols { trunc(s, cols) } else { format!("{}{}", " ".repeat(cols - w), s) }
}
...
pub fn blit_ch(buf: &mut Buffer, x: u16, y: u16, ch: char, style: Style) {
    if let Some(cell) = buf.cell_mut(Position::new(x, y)) {
        cell.set_char(ch); cell.set_style(style);
    }
}
...
pub fn fmt_cpt_parts(n: u64) -> (String, &'static str) {
    ...
    if      n < K { (format!("{}", n),   "B")  }
    else if n < M { (format!("{}", n/K), "Ki") }
    else if n < G { (format!("{}", n/M), "Mi") }
    else          { (format!("{}", n/G), "Gi") }
}

pub fn fmt_mode(mode: u32, is_dir: bool, is_link: bool) -> String {
    let ft  = if is_link { 'l' } else if is_dir { 'd' } else { '-' };
...
pub fn ls_style(e: &FileEntry, colors: &std::collections::HashMap<String, Style>) -> Style {
    ...
    if e.is_dir {
        ...
        let key = if sticky && other_write { "tw" }
                  else if other_write      { "ow" }
                  else if sticky           { "st" }
                  else                     { "di" };
        return colors.get(key).or_else(|| colors.get("di")).copied().unwrap_or(fallback);
    }
    if e.mode & 0o111 != 0 {
    ...
    if let Some(dot) = e.name.rfind('.')
        && dot > 0 {
            ...
            if let Some(s) = colors.get(&format!("ext:{}", ext)) { return *s; }
        }
    fallback
}
...
pub fn ui(frame: &mut Frame, app: &App) {
    let area = frame.area();
    let [menu_area, main_area] = Layout::vertical([
        Constraint::Length(1),
        Constraint::Min(10),
    ]).areas(area);
    ...
    if mw < LEFT_W + 20 || mh < HEADER_ROWS + 4 { return; }
    ...
    if app.tree_open {
        ...
        let file_area = Rect::new(sep_x + 1, list_area.y,
    ...
    let status_text = format!("{}  {:>3}/{:<3} {}", app.labels.f1_help, page, total, app.labels.page_unit);
...
fn render_menu(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let spans: Vec<Span> = app.menu_items.iter().flat_map(|(k, v)| {
        vec![Span::styled(k.as_str(), key_style), Span::styled(format!(":{} ", v), text_style)]
    }).collect();
...
fn render_header(frame: &mut Frame, main_area: Rect, inner: Rect, app: &App) {
    ...
    // ── Top border ───────────────────────────────────────────────────────
    {
        ...
        let vol_pre = "─ "; let vol_lbl = app.labels.vol_info;
        let vol_fill_n = LEFT_W.saturating_sub(sw(vol_pre) + sw(vol_lbl));
        let pas_pre = "─ "; let pas_lbl = app.labels.path_lbl; let pas_suf = " ";
        let ver_pre = "─── "; let ver_lbl = "f5h v0.1"; let ver_suf = " ─";
        let fixed = LEFT_W
                  + sw(pas_pre) + sw(pas_lbl) + sw(pas_suf)
                  + sw(&clock_s) + sw(ver_pre) + sw(ver_lbl) + sw(ver_suf);
        let mid_fill_n = mw.saturating_sub(fixed + 2);

        let mut x = mx;
        blit_ch(buf, x, my, '╭', cyan);                                  x += 1;
        blit(buf, x, my, vol_pre, sw(vol_pre), cyan);                     x += sw(vol_pre) as u16;
        blit(buf, x, my, vol_lbl, sw(vol_lbl), yellow);                   x += sw(vol_lbl) as u16;
        blit(buf, x, my, &"─".repeat(vol_fill_n), vol_fill_n, cyan);     x += vol_fill_n as u16;
        blit(buf, x, my, pas_pre, sw(pas_pre), cyan);                     x += sw(pas_pre) as u16;
        blit(buf, x, my, pas_lbl, sw(pas_lbl), yellow);                   x += sw(pas_lbl) as u16;
        blit(buf, x, my, pas_suf, sw(pas_suf), cyan);                     x += sw(pas_suf) as u16;
        blit(buf, x, my, &"─".repeat(mid_fill_n), mid_fill_n, cyan);     x += mid_fill_n as u16;
        blit(buf, x, my, &clock_s, sw(&clock_s), clock_st);               x += sw(&clock_s) as u16;
    ...
    // ── iy+0: volume (left) │ path + git branch (right) ──────────────
    {
        ...
        let branch_s = app.git_branch.as_ref()
        ...
        blit(buf, sx1 + 1, iy,
             &padr(&trunc(&path_s, path_avail), path_avail), path_avail, white);
        if branch_w > 0 {
            blit(buf, sx1 + 1 + path_avail as u16, iy, &branch_s, branch_w, yellow);
        }
    }
    ...
    let left_stat_cu = |buf: &mut Buffer, y: u16, lbl: &str, num: &str, unit: &str, sc: char| {
        let lw = sw(lbl); let nw = sw(num); let uw = sw(unit);
        let pad = LEFT_W.saturating_sub(lw + nw + uw);
        blit(buf, ix, y, lbl, lw, lc);
        if pad > 0 { blit(buf, ix + lw as u16, y, &" ".repeat(pad), pad, vc); }
    ...
    // ── iy+1: 空き (left) │ ─ ファイル情報 ─ title ───────────────────
    {
        ...
        let m_pre = "─ "; let m_lbl = app.labels.file_info; let m_suf = " ";
        let m_fill = info_w.saturating_sub(sw(m_pre) + sw(m_lbl) + sw(m_suf));
        let mut xm = sx1 + 1;
        blit(buf, xm, iy+1, m_pre, sw(m_pre), cyan);   xm += sw(m_pre) as u16;
        blit(buf, xm, iy+1, m_lbl, sw(m_lbl), yellow); xm += sw(m_lbl) as u16;
        blit(buf, xm, iy+1, m_suf, sw(m_suf), cyan);   xm += sw(m_suf) as u16;
    ...
    // ── iy+2: カレント (left) │ ファイル: name ──────────────────────
    {
        ...
        let ll = app.labels.curr; let llw = sw(ll);
        let cs_w = sw(&cnt_s); let iw2 = sw(app.labels.count_unit);
        ...
        blit(buf, ix + (llw + ts_w + 1 + cs_w) as u16,  iy+2, app.labels.count_unit, iw2,  lc);

        blit_ch(buf, sx1, iy+2, '│', cyan);
        let fname = cur.map(|e| e.name.as_str()).unwrap_or("");
        let fl = app.labels.file_lbl; let flw = sw(fl);
        blit(buf, sx1+1, iy+2, fl, flw, lc);
        blit(buf, sx1+1+flw as u16, iy+2,
             &padr(&trunc(fname, info_w.saturating_sub(flw)), info_w.saturating_sub(flw)),
             info_w.saturating_sub(flw), vc);
    }
    ...
    // ── iy+3: 合計 (left) │ 種別: file type ─────────────────────────
    {
        ...
        let sl = app.labels.kind; let slw = sw(sl);
        blit(buf, sx1+1, iy+3, sl, slw, lc);
        blit(buf, sx1+1+slw as u16, iy+3,
             &padr(&trunc(&app.file_type, info_w.saturating_sub(slw)), info_w.saturating_sub(slw)),
             info_w.saturating_sub(slw), vc);
    }
    ...
    // ── iy+4: 使用中 (left) │ サイズ/Blk/Inode/Links  [right: 修正/変更]
    {
        ...
        let mtime = cur.map(|e| format!("{} {}", e.date, e.time_full)).unwrap_or_default();
        let ctime = cur.map(|e| e.ctime_s.clone()).unwrap_or_default();
        let ts4_parts = [(app.labels.mtime, mtime.as_str()), (app.labels.ctime, ctime.as_str())];
        let ts4_w: usize = ts4_parts.iter().map(|(l, v)| sw(l) + sw(v)).sum();
        let ts4_x = sx1 + 1 + (info_w.saturating_sub(ts4_w)) as u16;

        let mut x4 = sx1 + 1;
        let sz_l = app.labels.size_lbl; let sz_lw = sw(sz_l);
        let sz_v = cur.map(|e| if e.is_dir { "< DIR >".to_string() } else { format!("{}", e.size) })
                      .unwrap_or_default();
        blit(buf, x4, iy+4, sz_l, sz_lw, lc); x4 += sz_lw as u16;
        blit(buf, x4, iy+4, &sz_v, sw(&sz_v), vc); x4 += sw(&sz_v) as u16;
        let blk_l = "  Blk:"; let blk_lw = sw(blk_l);
        let blk_v = cur.map(|e| format!("{}", e.blocks)).unwrap_or_default();
        blit(buf, x4, iy+4, blk_l, blk_lw, lc); x4 += blk_lw as u16;
        blit(buf, x4, iy+4, &blk_v, sw(&blk_v), vc); x4 += sw(&blk_v) as u16;
        let ino_l = "  Inode:"; let ino_lw = sw(ino_l);
        let ino_v = cur.map(|e| format!("{}", e.inode)).unwrap_or_default();
        blit(buf, x4, iy+4, ino_l, ino_lw, lc); x4 += ino_lw as u16;
        blit(buf, x4, iy+4, &ino_v, sw(&ino_v), vc); x4 += sw(&ino_v) as u16;
        let lnk_l = "  Links:"; let lnk_lw = sw(lnk_l);
        let lnk_v = cur.map(|e| format!("{}", e.nlink)).unwrap_or_default();
        blit(buf, x4, iy+4, lnk_l, lnk_lw, lc); x4 += lnk_lw as u16;
        blit(buf, x4, iy+4, &lnk_v, sw(&lnk_v), vc);

        let mut xt = ts4_x;
        for (lbl, val) in &ts4_parts {
            blit(buf, xt, iy+4, lbl, sw(lbl), lc); xt += sw(lbl) as u16;
            blit(buf, xt, iy+4, val, sw(val), date_st); xt += sw(val) as u16;
        }
    }

    // ── iy+5: 使用率 (left) │ 権限/所有  [right: 作成/参照] ────────────
    {
        let pct = if app.total_bytes > 0 { app.used_bytes * 100 / app.total_bytes } else { 0 };
        left_stat_cu(buf, iy+5, app.labels.usage, &format!("{}", pct), " %", '│');
        blit(buf, sx1+1, iy+5, &" ".repeat(info_w), info_w, vc);

        let birth = cur.and_then(|e| e.birth_s.as_deref())
                       .map(|s| s.to_string()).unwrap_or_else(|| "--".to_string());
        let atime = cur.map(|e| e.atime_s.clone()).unwrap_or_default();
        let ts5_parts = [(app.labels.birth, birth.as_str()), (app.labels.atime, atime.as_str())];
        let ts5_w: usize = ts5_parts.iter().map(|(l, v)| sw(l) + sw(v)).sum();
        let ts5_x = sx1 + 1 + (info_w.saturating_sub(ts5_w)) as u16;

        let mut x5 = sx1 + 1;
        let mode_s = cur.map(|e| fmt_mode(e.mode, e.is_dir, e.is_link))
                        .unwrap_or_else(|| "----------".to_string());
        let ml = app.labels.perm; let mlw = sw(ml);
        blit(buf, x5, iy+5, ml, mlw, lc); x5 += mlw as u16;
        blit(buf, x5, iy+5, &mode_s, sw(&mode_s), vc); x5 += sw(&mode_s) as u16;
        let ol = app.labels.own; let olw = sw(ol);
        blit(buf, x5, iy+5, ol, olw, lc); x5 += olw as u16;
        blit(buf, x5, iy+5, &app.owner_s, sw(&app.owner_s), vc);

        let mut xt = ts5_x;
        for (lbl, val) in &ts5_parts {
            blit(buf, xt, iy+5, lbl, sw(lbl), lc); xt += sw(lbl) as u16;
            blit(buf, xt, iy+5, val, sw(val), date_st); xt += sw(val) as u16;
        }
    }
...
fn render_run_dialog(frame: &mut Frame, app: &App) {
    let dlg = match app.run_dialog.as_ref() { Some(d) => d, None => return };
    let area = frame.area();
    ...
    let hint   = if app.lang_en { "  Enter:Run  Esc:Cancel" }
                 else           { "  Enter:実行  Esc:キャンセル" };
    let prompt = "> ";
    let pw = sw(prompt);
    let iw = dw - 2;
...
fn render_file_dialog(frame: &mut Frame, app: &App) {
    let dlg = match app.file_dialog.as_ref() { Some(d) => d, None => return };
    let area = frame.area();
    ...
    if let Some(ref prompt) = dlg.overwrite {
        if dlg.conflict_rename {
            ...
            let title = if app.lang_en { " Rename & Copy/Move " } else { " 名前変更して複写/移動 " };
            let label = if app.lang_en { "New name: " } else { "新ファイル名: " };
            let hint  = if app.lang_en { "  Enter:OK  Esc:Cancel" } else { "  Enter:実行  Esc:キャンセル" };
            ...
            blit(buf, dx + 1, dy + 2, &padr(&trunc(err_s, iw), iw), iw, err_st);
            blit_ch(buf, dx + dw as u16 - 1, dy + 2, '│', cyan);
            ...
            } else if app.lang_en { " Copy " } else { " 複写 " };
            ...
            let (lu, lo, lc, ln) = if dlg.kind == DialogKind::Move {
                if app.lang_en {
                    ("U:Move if newer", "O:Overwrite & move", "C:Rename then move", "N:Skip move")
                } else {
                    ("U:タイムスタンプが新しい時に移動する",
                     "O:上書きで移動する",
                     "C:名前を変更して移動する",
            ...
            blit_ch(buf, dx, dy + 1, '│', cyan);
            blit(buf, dx + 1, dy + 1, &padr(&trunc(&exist_msg, iw), iw), iw, yellow);
            blit_ch(buf, dx + dw as u16 - 1, dy + 1, '│', cyan);
            ...
            for (row, label) in [(3u16, lu), (4, lo), (5, lc), (6, ln)] {
                ...
                blit(buf, dx + 2, dy + row, &padr(&trunc(label, iw - 1), iw - 1), iw - 1, white);
    ...
    let (title, content, hint, show_input): (String, String, &str, bool) =
        match dlg.kind {
            DialogKind::DeleteConfirm => {
                let n = dlg.targets.len();
                let title = if app.lang_en {
                    if n == 1 { format!(" Delete [{}] ", dlg.targets[0]) }
                    else      { format!(" Delete [{} items] ", n) }
                } else if n == 1 { format!(" ファイルの削除 [{}] ", dlg.targets[0]) }
                else      { format!(" ファイルの削除 [{}件] ", n) };
                let content = if app.lang_en {
                    if n == 1 { format!("\"{}\" delete?", dlg.targets[0]) }
                    else      { format!("{} items delete?", n) }
                } else if n == 1 { format!("\"{}\" を削除しますか？", dlg.targets[0]) }
                else      { format!("{}件 を削除しますか？", n) };
                (title, content, "", false)
            }
            DialogKind::Rename => (
                if app.lang_en { " Rename ".to_string()  } else { " 名前変更 ".to_string() },
                if app.lang_en { "New name: ".to_string() } else { "新名前: ".to_string() },
                if app.lang_en { "  Enter:OK  Esc:Cancel" } else { "  Enter:実行  Esc:キャンセル" },
                true,
            ),
            DialogKind::Copy => (
                if app.lang_en { " Copy ".to_string() } else { " 複写 ".to_string() },
                if app.lang_en { "Dest: ".to_string() } else { "複写先: ".to_string() },
                if app.lang_en { "  Enter:OK  Esc:Cancel" } else { "  Enter:実行  Esc:キャンセル" },
                true,
            ),
            DialogKind::Move => (
                if app.lang_en { " Move ".to_string() } else { " 移動 ".to_string() },
                if app.lang_en { "Dest: ".to_string() } else { "移動先: ".to_string() },
                if app.lang_en { "  Enter:OK  Esc:Cancel" } else { "  Enter:実行  Esc:キャンセル" },
                true,
            ),
            DialogKind::Mkdir => (
                if app.lang_en { " New Dir ".to_string() } else { " 新規ディレクトリ ".to_string() },
                if app.lang_en { "Dir name: ".to_string() } else { "ディレクトリ名: ".to_string() },
                if app.lang_en { "  Enter:OK  Esc:Cancel" } else { "  Enter:実行  Esc:キャンセル" },
                true,
            ),
            DialogKind::Attr => (
                if app.lang_en { " Chmod ".to_string() } else { " 権限変更 ".to_string() },
                if app.lang_en { "Mode(octal): ".to_string() } else { "権限(8進数): ".to_string() },
                if app.lang_en { "  Enter:OK  Esc:Cancel" } else { "  Enter:実行  Esc:キャンセル" },
                true,
            ),
            DialogKind::CopyNewName => (
                if app.lang_en { " Copy (new name) ".to_string() } else { " 複写（新名前） ".to_string() },
                if app.lang_en { "New name: ".to_string() } else { "新ファイル名: ".to_string() },
                if app.lang_en { "  Enter:OK  Esc:Cancel" } else { "  Enter:実行  Esc:キャンセル" },
                true,
            ),
        };
    ...
    if show_input {
        ...
        blit(buf, dx + 1, dy + 1,
             &padr(&trunc(&content, iw), iw), iw, white);
    }
    blit_ch(buf, dx + dw as u16 - 1, dy + 1, '│', cyan);
...
fn render_error_msg(frame: &mut Frame, app: &App) {
    let msg = match app.error_msg.as_deref() { Some(m) => m, None => return };
    let area = frame.area();
    let dw   = (area.width as usize).clamp(40, 72);
    let dx   = ((area.width as usize).saturating_sub(dw) / 2) as u16;
...
fn render_file_list(frame: &mut Frame, area: Rect, app: &App, lh: usize) {
    if area.height == 0 || area.width == 0 || lh == 0 { return; }
    let cols    = app.cols();
    let pp      = cols * lh;
    ...
    for col in 0..cols {
        ...
        for row in 0..lh {
            ...
            let tag_st = if is_tagged { Style::default().fg(Color::Yellow) } else { Style::default() };
            ...
            blit(buf, col_x + PFX as u16, y,
                 &padr(&fmt_entry(e, entry_w, app.col_mode), entry_w), entry_w, file_style);
        }
    }
}
...
fn fmt_entry(e: &FileEntry, cw: usize, col_mode: u8) -> String {
    let name_disp = if e.is_dir && e.name != ".." { format!("{}/", e.name) }
                    else { e.name.clone() };
    let size_s = if e.is_dir { padl("<DIR>", SZ_W) } else { padl(&fmt_size(e.size), SZ_W) };
    let dt_s = if e.date.is_empty() { String::new() }
               else { format!("{} {}", e.date, e.time_str) };
