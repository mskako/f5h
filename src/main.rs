                ...
                && key.kind == KeyEventKind::Press {
                    ...
                    if app.run_dialog.is_some() {
                        ...
                        match (key.code, key.modifiers) {
                            ...
                            (KeyCode::Left,  _) => { if let Some(ref mut d) = app.run_dialog && d.cursor > 0 { d.cursor -= 1; } }
                            (KeyCode::Right, _) => { if let Some(ref mut d) = app.run_dialog && d.cursor < d.input.len() { d.cursor += 1; } }
                            (KeyCode::Home,  _) => { if let Some(ref mut d) = app.run_dialog { d.cursor = 0; } }
                            (KeyCode::End,   _) => { if let Some(ref mut d) = app.run_dialog { d.cursor = d.input.len(); } }
                            (KeyCode::Backspace, _) => {
                                if let Some(ref mut d) = app.run_dialog
                                    && d.cursor > 0 { d.cursor -= 1; d.input.remove(d.cursor); }
                            }
                            (KeyCode::Delete, _) => {
                                if let Some(ref mut d) = app.run_dialog
                                    && d.cursor < d.input.len() { d.input.remove(d.cursor); }
                            }
                                    ...
                                    match (key.code, key.modifiers) {
                                        ...
                                        (KeyCode::Enter, KeyModifiers::NONE) => {
                                            ...
                                            match app.resume_rename(&new_name, prompt) {
                                                Ok(None) => {
                                                    app.reload(); terminal.clear()?;
                                                }
                                                Ok(Some(p)) => {
                                                    dlg.conflict_rename = false;
                                                    dlg.input.clear(); dlg.cursor = 0;
                                                    dlg.error = None; dlg.overwrite = Some(p);
                                                    app.file_dialog = Some(dlg);
                                                }
                                                Err(e) => { dlg.error = Some(e.to_string()); app.file_dialog = Some(dlg); }
                                            }
                                        }
                                        (KeyCode::Left,  _) => { if dlg.cursor > 0 { dlg.cursor -= 1; } app.file_dialog = Some(dlg); }
                                        (KeyCode::Right, _) => { if dlg.cursor < dlg.input.len() { dlg.cursor += 1; } app.file_dialog = Some(dlg); }
                                        (KeyCode::Home,  _) => { dlg.cursor = 0; app.file_dialog = Some(dlg); }
                                        (KeyCode::End,   _) => { dlg.cursor = dlg.input.len(); app.file_dialog = Some(dlg); }
                                        (KeyCode::Backspace, _) => {
                                            if dlg.cursor > 0 { dlg.cursor -= 1; dlg.input.remove(dlg.cursor); }
                                            app.file_dialog = Some(dlg);
                                        }
                                        (KeyCode::Delete, _) => {
                                            if dlg.cursor < dlg.input.len() { dlg.input.remove(dlg.cursor); }
                                            app.file_dialog = Some(dlg);
                                        }
                                        (KeyCode::Char(c), KeyModifiers::NONE)
                                        | (KeyCode::Char(c), KeyModifiers::SHIFT) => {
                                    ...
                                    macro_rules! apply_resume {
                                        ($method:ident) => {{
                                            ...
                                            match app.$method(prompt) {
                                                Ok(None) => { app.reload(); terminal.clear()?; }
                                                Ok(Some(p)) => { dlg.error = None; dlg.overwrite = Some(p); app.file_dialog = Some(dlg); }
                                                Err(e) => { dlg.error = Some(e.to_string()); app.file_dialog = Some(dlg); }
                                            }
                                        }};
                                    }
                                    match key.code {
                                        ...
                                        KeyCode::Char('c') | KeyCode::Char('C') => {
                                            ...
                                            let fname: Vec<char> = dlg.overwrite.as_ref()
                                                ...
                                                .unwrap_or_default();
                                            let flen = fname.len();
                                            dlg.input = fname; dlg.cursor = flen;
                                        ...
                                        match (key.code, key.modifiers) {
                                            ...
                                            (KeyCode::Enter, KeyModifiers::NONE) => {
                                                ...
                                                if kind == DialogKind::Copy {
                                                    ...
                                                    let dc = dest.canonicalize().unwrap_or_else(|_| dest.clone());
                                                    let cc = app.current_dir.canonicalize().unwrap_or_else(|_| app.current_dir.clone());
                                                    if dc == cc {
                                                        if targets.len() > 1 {
                                                            dlg.error = Some(if app.lang_en {
                                                                "Multi-file same-dir copy unsupported".to_string()
                                                            } else {
                                                                "同ディレクトリへの複数ファイル複写は非対応です".to_string()
                                                            });
                                                            app.file_dialog = Some(dlg);
                                                        } else {
                                                            let orig: Vec<char> = targets[0].chars().collect();
                                                            let orig_len = orig.len();
                                                            app.file_dialog = Some(FileDialog {
                                                                kind: DialogKind::CopyNewName,
                                                                input: orig, cursor: orig_len,
                                                                targets, dest: Some(dest),
                                                                conflict_rename: false,
                                                                error: None, overwrite: None,
                                                            });
                                                        }
                                                    ...
                                                    let op_result = match kind {
                                                        DialogKind::Move        => app.begin_move(&input, &targets),
                                                        DialogKind::Rename      => app.exec_rename(&input, &targets).map(|_| None),
                                                        DialogKind::Mkdir       => app.exec_mkdir(&input).map(|_| None),
                                                        DialogKind::Attr        => app.exec_attr(&input, &targets).map(|_| None),
                                                        DialogKind::CopyNewName => {
                                                            let dest_dir = dlg.dest.clone().unwrap_or_else(|| app.current_dir.clone());
                                                            app.exec_copy_newname(&input, &targets, &dest_dir).map(|_| None)
                                                        }
                                                        _ => Ok(None),
                                                    };
                                                    match op_result {
                        ...
                        match action {
                            ...
                            Action::FirstEntry => { app.cursor = 0; app.update_file_info(); }
                            Action::LastEntry  => {
                                app.cursor = app.entries.len().saturating_sub(1);
                                ...
                                if let Some(e) = app.entries.get(app.cursor) {
                                    if e.is_dir {
                                        ...
                                        match app.enter_dir(&n) {
                                            Ok(()) => { if app.tree_open { app.tree_rebuild(); } }
                                        ...
                                        let ext = std::path::Path::new(&e.name)
                                            ...
                                            .unwrap_or("")
                                            .to_lowercase();
                                        let program = app.ext_programs.get(&ext)
                                            .cloned()
                            ...
                            Action::HomeDir => {
                                if let Some(home) = std::env::var_os("HOME") {
                                    ...
                                    match app.load_entries() {
                                        Ok(()) => {
                                            app.update_file_info();
                                            if app.tree_open { app.tree_rebuild(); }
                                        }
                                        Err(e) => {
                            ...
                            Action::Copy => {
                                ...
                                if !targets.is_empty() {
                                    let input: Vec<char> = app.current_dir.to_string_lossy().chars().collect();
                                    let cursor = input.len();
                                    app.file_dialog = Some(FileDialog { kind: DialogKind::Copy, input, cursor, targets, dest: None, conflict_rename: false, error: None, overwrite: None });
                                }
                            }
                            Action::Move => {
                                let targets = app.collect_op_targets();
                                if !targets.is_empty() {
                                    let input: Vec<char> = app.current_dir.to_string_lossy().chars().collect();
                                    let cursor = input.len();
                                    app.file_dialog = Some(FileDialog { kind: DialogKind::Move, input, cursor, targets, dest: None, conflict_rename: false, error: None, overwrite: None });
                                }
                            }
                            Action::Delete => {
                                let targets = app.collect_op_targets();
                                if !targets.is_empty() {
                                    app.file_dialog = Some(FileDialog { kind: DialogKind::DeleteConfirm, input: vec![], cursor: 0, targets, dest: None, conflict_rename: false, error: None, overwrite: None });
                                }
                            }
                            Action::Rename => {
                                let targets = app.collect_op_targets();
                                if let Some(fname) = targets.first() {
                                    ...
                                    let cursor = input.len();
                                    app.file_dialog = Some(FileDialog { kind: DialogKind::Rename, input, cursor, targets: vec![fname.clone()], dest: None, conflict_rename: false, error: None, overwrite: None });
                                }
                            }
                            Action::Mkdir => {
                                app.file_dialog = Some(FileDialog { kind: DialogKind::Mkdir, input: vec![], cursor: 0, targets: vec![], dest: None, conflict_rename: false, error: None, overwrite: None });
                            }
                            Action::Attr => {
                                let targets = app.collect_op_targets();
                                if !targets.is_empty() {
                                    let mode_s: Vec<char> = app.entries.get(app.cursor)
                                        .map(|e| format!("{:04o}", e.mode & 0o7777))
                                        .unwrap_or_default().chars().collect();
                                    let cursor = mode_s.len();
                                    app.file_dialog = Some(FileDialog { kind: DialogKind::Attr, input: mode_s, cursor, targets, dest: None, conflict_rename: false, error: None, overwrite: None });
                                }
                            }
                            Action::DirJump  => { /* TODO */ }
                            Action::TreeToggle => {
