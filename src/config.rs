...
impl UiColors {
    pub fn from_config(c: &ColorsConfig) -> Self {
        let mk = |s: &str, default: Color| {
            if s.is_empty() { Style::default().fg(default) }
            else            { Style::default().fg(parse_color(s)) }
        };
        UiColors {
...
pub enum Action {
    ...
    PageUp, PageDown,
    Enter, ParentDir, HomeDir,
    TagMove, TagAll,
    Quit,
    ColMode1, ColMode2, ColMode3, ColMode5,
    Mkdir, DirJump,
    TreeToggle,
    Edit,
    Run,
    Copy, Move, Delete, Rename, Attr,
}
...
pub fn key_label(keymap: &KeyMap, action: Action) -> String {
    for (&(code, mods), &act) in keymap {
        if act != action || mods != KeyModifiers::NONE { continue; }
        match code {
            // 矢印キーはナビゲーション専用のため除外
